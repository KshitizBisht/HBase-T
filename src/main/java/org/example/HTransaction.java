  ...
  public void releaseLock(){
    ...
    for (Entry<String, UpdatesTransactionDetails> set : this.columnToValue.entrySet()) {
      try {
        ...
        Delete delete = new Delete(Bytes.toBytes(set.getValue().rowId));


        delete.addFamily(rowLockFamily);
        table.delete(delete);
        System.out.println("Deleting row lock");
      } catch (Exception e) {
        System.out.println("Row lock cannot be deleted");
        System.out.println(e.getMessage());
      }
    }
  }
...
//    Scan scan = new Scan();
//    long size = conn.getTable(tableName).getScanner(scan)
//            .spliterator()
//            .mapToLong(result -> result.rawCells().length)
//            .sum();
//    System.out.println("Size of table data: " + size + " bytes");
//    conn.close();
//    // try {
...
//    // }
    try {
      ...
      Admin admin = conn.getAdmin();
      HTransaction htx = new HTransaction(conn, admin);
      htx.newPut("T2", "row2", Bytes.toBytes("cf"), Bytes.toBytes("cq"), Bytes.toBytes("new value"));
      Thread.sleep(1000);
      htx.newPut("T2", "row1", Bytes.toBytes("cf"), Bytes.toBytes("cq"), Bytes.toBytes("new Value"));
      //    Thread.sleep(1000);
